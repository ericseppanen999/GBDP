...
def _stage_fetch_schedules(start, end, cfg, writer, cache, force):
...
def _stage_fetch_rosters(start, end, cfg, writer, cache, force):
    mlb = MlbStatsApiConnector(writer, cache, cfg["mlb_statsapi"]["base_url"])
    npb = NpbSpaiaConnector(writer, cache, cfg["npb_spaia"]["base_url"])
...
def _stage_fetch_transactions(start, end, cfg, writer, cache, force):
    mlb = MlbStatsApiConnector(writer, cache, cfg["mlb_statsapi"]["base_url"])
...
def _stage_fetch_games(start, end, cfg, writer, cache, force):
    mlb = MlbStatsApiConnector(writer, cache, cfg["mlb_statsapi"]["base_url"])
    npb = NpbSpaiaConnector(writer, cache, cfg["npb_spaia"]["base_url"])
    for p in mlb.list_partitions(start, end, "schedule"):
        payload = mlb.fetch_partition(p)
...
def _stage_fetch_pbp(start, end, cfg, writer, cache, force):
    npb = NpbSpaiaConnector(writer, cache, cfg["npb_spaia"]["base_url"])
...
def _stage_fetch_pitches(start, end, cfg, writer, cache, force):
...
def _stage_silver(start, end, cfg, writer, cache, force):
...
def _stage_identity(start, end, cfg, writer, cache, force):
    resolve_identity(start, end, force=force)


def _stage_gold(start, end, cfg, writer, cache, force):
    publish_gold(start, end, force=force)


def _stage_quality(start, end, cfg, writer, cache, force):
    run_quality_checks(start, end, force=force)


def _stage_audit(start, end, cfg, writer, cache, force):
