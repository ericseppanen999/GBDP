...
class KboLocalConnector(BaseConnector):
    """
    Local-file connector for KBO data.
    Expects JSON files in manual/kbo/<entity>/dt=YYYY-MM-DD/*.json
    """
    ...
    def fetch_partition(self, partition: Partition) -> RawPayload:
        base = manual_root() / "kbo" / partition.entity / f"dt={partition.dt}"
        records = []
        if path_exists(base):
