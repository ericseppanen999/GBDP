...
class LmbLocalConnector(BaseConnector):
    """
    Local-file connector for LMB data.
    Expects JSON files in manual/lmb/<entity>/dt=YYYY-MM-DD/*.json
    """
    ...
    def fetch_partition(self, partition: Partition) -> RawPayload:
        base = manual_root() / "lmb" / partition.entity / f"dt={partition.dt}"
        records = []
        if path_exists(base):
